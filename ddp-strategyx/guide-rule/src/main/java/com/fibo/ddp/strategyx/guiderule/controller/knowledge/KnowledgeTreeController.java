package com.fibo.ddp.strategyx.guiderule.controller.knowledge;
...
import com.fibo.ddp.common.utils.constant.strategyx.Status;
...
public class KnowledgeTreeController {
    ...
    @Autowired
    private com.fibo.ddp.common.service.strategyx.knowledge.RuleService ruleService2;
    ...
    public ResponseEntityDto<Object> save(@RequestBody Map<String, Object> param) {
        ...
        boolean flag = knowledgeTreeService.insertTree(k);
        if (flag) {
            param.put("result", 1);
        } else {
            param.put("result", -1);
     ...
     * @apiGroup knowledge
     * @apiVersion 2.0.0
     * @apiParam {Integer} userId 节点id
     * @apiParam {String} name 节点名称
     * @apiParam {Integer} engineId engineId
     * @apiSuccess {String} status 状态: 1成功, 0失败
     * @apiParamExample {json} 请求示例：
     * 修改：
     * {"userId":1162,"name":"文件夹1127","engineId":""}
     * 删除：
     * {"userId":1162,"status":"-1","type":"1","engineId":"","parentId":0}
     * @apiSuccessExample {json} 成功返回数据示例：
     * {"status":"1","error":"00000000","msg":null,"data":{"userId":1162,"name":"文件夹1127","engineId":"","userId":135,"organId":46,"result":1}}
     */
    ...
    public ResponseEntityDto<Object> update(@RequestBody Map<String, Object> param) {
        ...
        KnowledgeTree k = knowledgeTreeService.findById(id);
        ...
        if (param.containsKey("status")) {
            ...
            if (k.getTreeType() == 0) {
                List<Rule> rlist = ruleService2.getRuleList(map);
...
//                    s.ruleService.updateRuleStatus(map);
            ...
            if (k.getTreeType() == 1) {
                List<Scorecard> slist = scorecardService.getScorecardList(map);
                if (slist != null && slist.size() > 0) {
                    for (Scorecard s : slist) {
                        idList.add(s.getId());
                    }
                    map.put("status", status);
                    map.put("idList", idList);

                    scorecardService.updateScorecardStatus(idList, status);
                }
            }
            if (k.getTreeType() == 3) {
//                decisionTablesService.updateStatus(idList.toArray(new Long[idList.size()]),status);
        ...
        boolean flag = knowledgeTreeService.updateTree(k);
        if (flag) {
            param.put("result", 1);
        } else {
            param.put("result", -1);
    ...
    /**
     * getInputParam:(根据用户角色或权限,获取所需参数集合)
     *
     * @return paramMap
     */
